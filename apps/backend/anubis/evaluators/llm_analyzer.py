...
class LLMAnalyzer:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError(
                "GOOGLE_API_KEY must be set in environment or passed as parameter"
            )

        self.client = genai.Client(api_key=self.api_key)
        self.model = "gemini-2.5-flash"  # Using Gemini 2.5 Pro for code analysis
    ...
    def analyze(self, code: str) -> Dict[str, any]:
        ...
        try:
            ...
            response = self.client.models.generate_content(
                model=self.model, contents=analysis_prompt
            )
            ...
            return {
                ...
                "language": (
                    detected_language if detected_language != "unknown" else "unknown"
                ),
            }
    ...
    def _build_analysis_prompt(
        self, code: str, detected_language: str = "unknown"
    ) -> str:
        """
        ...
        """
        language_hint = (
            f" (The code appears to be {detected_language})"
            if detected_language != "unknown"
            else ""
        )
